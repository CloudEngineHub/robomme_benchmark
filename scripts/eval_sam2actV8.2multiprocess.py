...
def evaluate_environment(env_id, api_url, max_steps, dataset_root):
    """
    评估单个环境的所有episodes
    
    该函数处理指定环境的所有episode评估任务，包括：
    1. 读取metadata文件获取episode配置
    2. 对每个episode创建环境并执行评估
    3. 调用API获取动作并执行
    4. 评估任务完成情况
    
    Args:
        env_id: 环境ID（任务类型，如"BinFill"）
        api_url: API服务器地址
        max_steps: 每个episode的最大执行步数
        dataset_root: 数据集根目录路径
        
    Returns:
        dict: 包含评估结果的字典，包括环境ID和处理状态
    """
    try:
        print(f"[{env_id}] Starting evaluation with API: {api_url}")
        ...
        for episode_record in episode_records:
            ...
            print(f"[{env_id}] --- Running simulation for episode:{episode}, seed: {seed}, difficulty: {difficulty} ---")
            ...
            for step in range(max_steps):
                print(f"[{env_id}] Step {step}/{max_steps}")
        ...
        return {"env_id": env_id, "status": "completed", "episodes_processed": len(episode_records)}
        
    except Exception as e:
        print(f"[{env_id}] Error in evaluate_environment: {e}")
        import traceback
        traceback.print_exc()
        return {"env_id": env_id, "status": "failed", "error": str(e)}
...
def main():
    ...
    2. 为每个环境分配不同的API服务器地址（不同端口）
    3. 使用多进程并行执行所有环境的评估任务
    4. 收集并输出评估结果
    
    工作流程：
    - 从默认API URL提取基础地址（IP和协议）
    - 为每个环境生成递增端口的URL（8001, 8002, 8003, ...）
    - 使用ProcessPoolExecutor并行执行所有环境的评估
    - 每个环境独立处理其所有episodes
    """
    # ========== 命令行参数解析 ==========
    parser = argparse.ArgumentParser(description='Run evaluation using SAM2ACT Agent API with parallel processing')
    # API服务器地址，默认本地8001端口（将作为基础地址，端口会递增）
    parser.add_argument('--api_url', type=str, default='http://141.212.48.176:8001', help='Base API URL (port will be incremented for each environment)')
    # 每个episode的最大执行步数，默认40步
    parser.add_argument('--max_steps', type=int, default=40, help='Max steps per episode')
    args = parser.parse_args()
    
    # 提取参数值
    base_api_url = args.api_url  # API服务器基础URL
   ...
   "RouteStick"
    ...
    # ========== 生成API URL映射 ==========
    # 从基础URL提取协议和主机地址
    # 例如：http://141.212.48.176:8001 -> http://141.212.48.176
    # 解析URL格式：http://host:port 或 http://host
    if '://' in base_api_url:
        protocol, rest = base_api_url.split('://', 1)
        if ':' in rest:
            ...
            try:
                ...
                start_port = 8001
            base_host = f"{protocol}://{host}"
        else:
            base_host = f"{protocol}://{rest}"
            start_port = 8001
    else:
        # 如果没有协议，假设是http
        if ':' in base_api_url:
            host, port_str = base_api_url.rsplit(':', 1)
            try:
                start_port = int(port_str)
            except ValueError:
                start_port = 8001
            base_host = f"http://{host}"
        else:
            base_host = f"http://{base_api_url}"
            start_port = 8001
    
    # 为每个环境生成不同的API URL（端口递增）
    api_urls = []
    for idx, env_id in enumerate(env_id_list):
        port = start_port + idx
        api_url = f"{base_host}:{port}"
        api_urls.append(api_url)
        print(f"[Main] Environment {env_id} will use API: {api_url}")
    
    # ========== 并行执行所有环境的评估 ==========
    print(f"\n[Main] Starting parallel evaluation for {len(env_id_list)} environments...")
    print(f"[Main] Using ProcessPoolExecutor with {len(env_id_list)} workers\n")
    
    with ProcessPoolExecutor(max_workers=len(env_id_list)) as executor:
        # 提交所有环境的评估任务
        future_to_env = {
            executor.submit(evaluate_environment, env_id, api_url, max_steps, dataset_root): env_id
            for env_id, api_url in zip(env_id_list, api_urls)
        }
        ...
        for future in as_completed(future_to_env):
            ...
            try:
                ...
                print(f"\n[Main] Environment {env_id} completed: {result.get('status', 'unknown')}")
            except Exception as exc:
                print(f"\n[Main] Environment {env_id} generated an exception: {exc}")
                import traceback
                traceback.print_exc()
                results.append({"env_id": env_id, "status": "exception", "error": str(exc)})
    ...
    for result in results:
        ...
        if status == "completed":
            ...
            print(f"  {env_id}: {status} ({episodes} episodes processed)")
        elif status == "skipped":
            reason = result.get("reason", "unknown")
            print(f"  {env_id}: {status} (reason: {reason})")
        else:
            error = result.get("error", "unknown error")
            print(f"  {env_id}: {status} (error: {error})")
    print("="*60)
    print("[Main] All evaluations completed!")

...
def solve_and_evaluate_from_command(env, planner, env_id, seg_vis, seg_raw, base_frames, wrist_frames, command_dict, use_visualize=True):
    ...
    if "action" not in command_dict: return None, None
    if target_action is None: return None, None
...
class EpisodeConfigResolverForOraclePlanner:
    ...
    """
    def __init__(self, dataset_root, gui_render=True, max_steps_without_demonstration=3000):
    ...
    def initialize_episode(self, env_id, episode_idx):
        ...
        """
        dataset_path = self.dataset_root / f"record_dataset_{env_id}.h5"
        ...
        self.current_dataset_file = h5py.File(dataset_path, "r")
        self.current_env_id = env_id

        resolver = EpisodeConfigResolver(
            env_id=env_id,
            dataset=self.current_dataset_file,
        ...
        return env, planner, color_map, language_goal, tasks
...
def main():
    dataset_root = "/data/hongzefu/dataset_demonstration"
    num_episodes = 100
    ...
    try:
        for env_id in env_id_list:
            for episode in range(num_episodes):
                ...
                # --- WRAPPED INITIALIZATION START ---
                env, planner, color_map, language_goal, tasks = oracle_resolver.initialize_episode(env_id, episode)
                
                # Check for skipped envs or empty tasks
                if env is None: 
                    continue # Dataset missing
                
                if not tasks:
                    env.close()
                    continue
                # --- WRAPPED INITIALIZATION END ---

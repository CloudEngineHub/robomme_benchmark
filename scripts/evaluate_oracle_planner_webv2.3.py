...
class OracleGradioService:
    ...
    def _get_base_view_snapshot(self):
        """
        Fetch the base camera view from the environment observation.
        """
        if not self.env:
            return None
        ...
        try:
            ...
            # print(f"[Base View] Failed: {e}")
            pass
            
        return None

    def _get_streaming_views(self):
        return self._get_viewer_snapshot(), self._get_base_view_snapshot(), self._get_wrist_view_snapshot()
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    ...
                    base_display = gr.Image(label="Base Camera", interactive=False)

...
# [New] 支持并行多进程回放，并在两个 GPU 间交替分配任务。
...
def init_worker(gpu_id: int):
    """
    Worker 进程初始化函数，设置 CUDA_VISIBLE_DEVICES。
    """
...
def evaluate_episode(
    ...
    单个 Episode 的评估逻辑。
    """
    # 重新构建 Envs 和 Resolver (避免跨进程传递复杂对象)
    ...
    try:
        ...
        status = "成功" if episode_success else "结束"
        if not episode_success and info.get("fail", False):
            status = "失败"
        return f"[{env_id}] episode {episode} {status} (step {step})"

    except (FileNotFoundError, KeyError) as exc:
        return f"[{env_id}] episode {episode} 数据缺失，跳过。{exc}"
    except Exception as exc:
        # import traceback
        # traceback.print_exc()
        return f"[{env_id}] episode {episode} 回放异常，跳过。{exc}"
    finally:
...
def process_env_id(env_id: str, max_workers_total: int):
    # 简单计算 episode count (不实例化 env_builder 避免开销，或者轻量实例化)
    # 为了获取 episode_count，我们需要实例化一次 env_builder
    # 但我们只需要 metadata 解析部分
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=mw0, initializer=init_worker, initargs=(0,)) as executor0, \
        ...
        for episode in range(episode_count):
            # 交替分配：偶数 -> GPU0, 奇数 -> GPU1
            if episode % 2 == 0:
                executor = executor0
            else:
                executor = executor1
                # 如果用户只分配了 1 个 worker (mw1=0)，则全部压给 executor0 ?
                # 这种情况下 mw1 应该为 0。
                if mw1 == 0:
...
def main():
    # 强制使用 spawn，避免 PyTorch/CUDA fork 问题
